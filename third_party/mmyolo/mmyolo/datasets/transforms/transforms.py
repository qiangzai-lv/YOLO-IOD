        ...
        for instance in results.get('instances', []):
            if instance['ignore_flag'] == 0:
                ...
                if 'bbox_score' in instance:
                    bbox_scores.append(instance['bbox_score'])
        if len(bbox_scores) > 0:
            results['gt_bboxes_scores'] = np.array(bbox_scores, dtype=np.float32)
...
class YOLOv5RandomAffine(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        if num_bboxes:
            ...
            results['gt_bboxes_labels'] = results['gt_bboxes_labels'][
                valid_index]
            if 'gt_bboxes_scores' in results:
                try:
                    results['gt_bboxes_scores'] = results['gt_bboxes_scores'][valid_index]
                except Exception as e:
                    print(e)
                    print("valid_index", valid_index)
                    print('gt_bboxes_scores', results['gt_bboxes_scores'])
...
class YOLOv5CopyPaste(BaseTransform):
    ...
    def transform(self, results: dict) -> Union[dict, None]:
        ...
        gt_bboxes_scores = results.get('gt_bboxes_scores', None)
        img = results['img']
        img_h, img_w = img.shape[:2]
        ...
        if gt_bboxes_scores is not None:
            gt_bboxes_scores = np.concatenate(
                (gt_bboxes_scores, gt_bboxes_scores[valid_inds]), axis=0)
        ...
        if gt_bboxes_scores is not None:
            results['gt_bboxes_scores'] = gt_bboxes_scores
        results['gt_masks'] = gt_masks
