...
from yolo_world.models.dense_heads.yolo_world_head import BNContrastiveHead, ContrastiveHead, RepBNContrastiveHead


@MODELS.register_module()
class YOLOWorldUnHeadModule(YOLOv8HeadModule):
    ...
    def __init__(self,
        ...
        self.known_cls_num = known_cls_num
    ...
    def forward_single(self, img_feat: Tensor, txt_feat: Tensor,
        ...
        txt_feats_known = txt_feat[:, :self.known_cls_num, :]
        txt_feats_unknown = txt_feat[:, self.known_cls_num:, :]
        cls_logit_unknown = cls_contrast(cls_embed, txt_feats_unknown)
        cls_logit_known = cls_contrast(cls_embed, txt_feats_known)
        ...
        if self.training:
            return cls_logit_known, cls_logit_unknown, bbox_preds, bbox_dist_preds
        else:
            return cls_logit_known, bbox_preds


@MODELS.register_module()
class YOLOWorldUnHead(YOLOv8Head):
    """YOLO-World Head
    """

    def __init__(self, world_size=-1, known_cls_num=40, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
    ...
    def loss_by_feat_single(
        ...
        # pred info
        flatten_cls_preds = [
            cls_pred.permute(0, 2, 3, 1).reshape(num_imgs, -1, num_cls)
        ...
        self.assigner.num_classes = num_cls
    ...
    def loss_by_feat(
            self,
            cls_scores_known: Sequence[Tensor],
            cls_scores_unknown: Sequence[Tensor],
            bbox_preds: Sequence[Tensor],
            bbox_dist_preds: Sequence[Tensor],
            batch_gt_instances: Sequence[InstanceData],
            batch_img_metas: Sequence[dict],
        ...
        known_batch_gt_instances = []
        unknown_batch_gt_instances = []
        for gt_instance in batch_gt_instances:
            if gt_instance[1] >= self.known_cls_num:
                gt_instance[1] -= self.known_cls_num
                unknown_batch_gt_instances.append(gt_instance)
            else:
                known_batch_gt_instances.append(gt_instance)
        known_batch_gt_instances = torch.stack(known_batch_gt_instances, dim=0)
        if len(unknown_batch_gt_instances) == 0:
            unknown_batch_gt_instances = torch.empty(0, 6).to(known_batch_gt_instances.device)
        else:
            unknown_batch_gt_instances = torch.stack(unknown_batch_gt_instances, dim=0)

        unknown_cls_num = self.num_classes - self.known_cls_num

        loss = self.loss_by_feat_single(cls_scores_known, bbox_preds, bbox_dist_preds, known_batch_gt_instances,
                                        batch_img_metas, batch_gt_instances_ignore, self.known_cls_num)
        loss_unknown = self.loss_by_feat_single(cls_scores_unknown, bbox_preds, bbox_dist_preds,
                                                unknown_batch_gt_instances, batch_img_metas, batch_gt_instances_ignore,
                                                unknown_cls_num)

        loss['loss_cls_unknown'] = loss_unknown['loss_cls'] * 0.4
        loss['loss_bbox_unknown'] = loss_unknown['loss_bbox'] * 0.2
        loss['loss_dfl_unknown'] = loss_unknown['loss_dfl'] * 0.2
