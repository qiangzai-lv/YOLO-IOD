...
@MODELS.register_module()
class HuggingCacheCLIPLanguageBackbone(BaseModule):
    ...
    def forward(self, text: List[List[str]]) -> Tensor:
        ...
        uncached_texts = [t for t in flat_text if t not in self.cache]
...
@MODELS.register_module()
class HuggingCacheCLIPCLSPSLanguageBackbone(BaseModule):

    def __init__(self,
                 model_name: str,
                 frozen_modules: Sequence[str] = (),
                 dropout: float = 0.0,
                 cls_unknown_weight_path: str = None,
                 init_cfg: OptMultiConfig = None) -> None:
    ...
    def forward(self, text: List[List[str]]) -> Tensor:
        ...
        uncached_texts_without_unknown = []
        for t in uncached_texts:
            if t in self.cls_unknown_feature_map:
                if 'unknown' in t :
                    assert t in self.cls_unknown_feature_map
                fea = self.cls_unknown_feature_map[t]
                fea = fea.to(self.model.device)
                self.cache[t] = fea.detach()
            else:
                uncached_texts_without_unknown.append(t)
        uncached_texts = uncached_texts_without_unknown
